...
"""MCP server for CESSDA research datasets using HTTP/SSE transport."""
...
def main():
    ...
    log_with_context(
        logger,
        "INFO",
        "Starting CESSDA MCP server via HTTP/SSE",
        host=host,
        port=port,
    )
    ...
    async def health_check(request):
        from starlette.responses import JSONResponse
        return JSONResponse({"status": "healthy", "service": "mcp-cessda-datasets", "version": "0.1.0"})
